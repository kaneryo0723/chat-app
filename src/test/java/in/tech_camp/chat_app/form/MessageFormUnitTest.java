...
public class MessageFormUnitTest {
  ...
  class メッセージが投稿できる場合 {
    ...
    public void contentとimageが存在していれば保存できる () {
      messageForm.validateMessage(bindingResult);//contentとimageどっちも空だったらエラーメッセージを出力するメソッドを記述
      //never():メソッドが呼ばれていないことを検証。つまり、validateMessage()が呼び出されていないことを確認。
      //bindingResult.rejectValue()が呼び出されていないことを確認する。
